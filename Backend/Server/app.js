...
let apiKey = 'IntutXFyKb3HKlLt';
let merchantId = '43043';
    ...
    */
    if (request.method == 'POST' && request.url == '/') {
        ...
        request.on('data', (chunk) => {
            ...
            // request options for token get
            // uses Basic http auth
            var options = {
                url: 'https://' + merchantId + ':' + apiKey + '@api.xsolla.com/merchant/v2/merchants/' + merchantId + '/token',
                method: 'POST',
                headers: {
            ...
            // process request
            makeRequest(options, (err, tokenResponse, body) => {
                if (err) throw err;

                if (tokenResponse) {
                    response.end(body.token);
                }
            });
        });
    }
